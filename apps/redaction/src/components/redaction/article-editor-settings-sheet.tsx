...
import { CalendarClock, Check, ChevronRight, GripVertical, X, Zap } from 'lucide-react';
...
function CategoryPickerSheet({
  ...
  'categories' | 'selectedCategoryIds' | 'onToggleCategory' | 'onSetPrimaryCategory'
> & {
  open: boolean;
  onClose: () => void;
}) {
  ...
  return (
    <div className="fixed inset-0 z-[70] flex flex-col justify-end lg:items-center lg:justify-center lg:p-6">
      <button
        ...
        aria-label="Fermer les rubriques"
        onClick={onClose}
      />
      <div
        role="dialog"
        ...
        className="relative max-h-[min(92dvh,720px)] w-full overflow-y-auto rounded-t-2xl bg-background shadow-2xl pb-[max(1rem,env(safe-area-inset-bottom))] lg:max-h-[min(85vh,720px)] lg:max-w-xl lg:rounded-2xl"
      >
        <div className="sticky top-0 z-10 flex items-center justify-between border-b border-border bg-background/95 px-4 py-3 backdrop-blur">
          <div>
            <h3 id="category-picker-title" className="font-display text-base font-bold">
              Choisir les rubriques
            </h3>
            <p className="mt-0.5 text-xs text-muted-foreground">
              La rubrique principale détermine l’URL sur le site.
            </p>
          </div>
          <button
            type="button"
              ...
              <div className="min-w-0">
                ...
                <p className="mt-0.5 text-xs text-muted-foreground">
                  {selectedCategoryIds[0]
                    ? `Principale : ${
                        categories.find((category) => category.documentId === selectedCategoryIds[0])?.name ??
                        'Rubrique'
                      }`
                    : 'Ouvrir le sélecteur de rubriques'}
                </p>
              </div>
