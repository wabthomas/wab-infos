...
> **Miniature absente ?** Facebook récupère l’aperçu via les balises Open Graph de l’URL (`og:image`, etc.). Si la vignette manque, vérifiez l’URL dans [Sharing Debugger](https://developers.facebook.com/tools/debug/) puis republiez (article sans `facebookPostedAt`).
